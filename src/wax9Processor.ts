...
export class Wax9Processor {
    ...
    update(sn: number, ax: number, ay: number, az: number, gx: number, gy: number, gz: number, mx: number, my: number, mz: number) : Wax9Data {
        let acc = new Vector3(ax, ay, az);
        let gyro = new Vector3(gx, gy, gz);
        let mag = new Vector3(mx, my, mz);
        ...
        return new Wax9Data(sn, new Date(), acc, gyro, mag, euler, grav, linAcc);
    }

    updateFromBytes(data:Buffer) : Wax9Data {
    ...
    static _quatToEuler(q: Vector4) : Vector3
    {
    ...
    static _quatToGravity(q: Vector4) : Vector3 {
        let gx = 2 * (q.y * q.w - q.x * q.z);
        let gy = 2 * (q.x * q.y + q.z * q.w);
        let gz = q.x * q.x - q.y * q.y - q.z * q.z + q.w * q.w;

        return new Vector3(-gy, -gz, -gx);
    }

    static _toRadians(x: number) : number {
        return x * Math.PI / 180.0;
    }

    static _matrixLoadZXYRotation(phi: number, theta: number, psi: number) 
    {
    ...
    static _matrixLoadZXYInverseRotation(phi: number, theta: number, psi: number)
    {
        let rotZ = Wax9Processor._matrixLoadZRotation(phi);    // Z: phi (roll)
        let rotX = Wax9Processor._matrixLoadXRotation(theta);  // X: theta (pitch)
        let rotY = Wax9Processor._matrixLoadYRotation(psi);    // Y: psi (yaw)
    ...
    static _matrixLoadXRotation(angleRadians: number) : number[]
    {
        let dest = [];
    ...
    static _matrixLoadYRotation(angleRadians: number) : number[]
    {
        let dest = [];
    ...
    static _matrixLoadZRotation(angleRadians: number) : number[]
    {
        let dest = [];
    ...
    static _matrixMultiply(matrix1: number[], matrix2: number[]) : number[]
    {
        let result = [];
    ...
    static _point3MultiplyMatrix(point: number[], matrix: number[]) : Vector3
    {
